...
## association statistics for Hair and Eye color
assocstats(HairEye)
...
## Figure 1.3.: Donner party data, showing the relationship between age and survival.
...
# new code
    ggplot(Donner, aes(age, survived)) + theme_bw() +
        ...
        stat_smooth(method = "glm", method.args = list(family = binomial),
                    formula = y ~ x, fill="blue", alpha = 0.3, size=2)

}

## Figure 1.4.: Donner party data, showing other model-based smoothers for the relationship between age and survival. Left: using a natural spline; right: using a non-parametric loess smoother.', fig.pos="!b"----
library(splines)
ggplot(Donner, aes(age, survived)) + theme_bw() +
  geom_point(position = position_jitter(height = 0.02, width = 0)) +
